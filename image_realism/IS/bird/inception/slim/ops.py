...
from inception.slim import losses, scopes, variables
...
def batch_norm(
    ...
    with tf.variable_scope(scope, "BatchNorm", [inputs], reuse=reuse):
        ...
        if center:
            beta = variables.variable(
                "beta", params_shape, initializer=tf.zeros_initializer(), trainable=trainable, restore=restore
            )
        if scale:
            gamma = variables.variable(
                "gamma", params_shape, initializer=tf.ones_initializer(), trainable=trainable, restore=restore
            )
        ...
        moving_variance = variables.variable(
            "moving_variance",
            params_shape,
    ...
    with tf.variable_scope(scope, "Conv", [inputs], reuse=reuse):
        ...
        if batch_norm_params is not None:
            ...
            biases = variables.variable(
                "biases", shape=bias_shape, initializer=bias_initializer, trainable=trainable, restore=restore
            )
...
def max_pool(inputs, kernel_size, stride=2, padding="VALID", scope=None):
    ...
    with tf.name_scope(scope, "MaxPool", [inputs]):
        ...
        return tf.nn.max_pool(
            inputs, ksize=[1, kernel_h, kernel_w, 1], strides=[1, stride_h, stride_w, 1], padding=padding
        )
