...
from inception.slim import ops, scopes, variables
...
class ConvTest(tf.test.TestCase):
    ...
    def testCreateConvWithWD(self):
        ...
        with self.test_session() as sess:
            ...
            self.assertEquals(wd.op.name, "Conv/weights/Regularizer/L2Regularizer/value")
    ...
    def testCreateConvWithoutWD(self):
        ...
        with self.test_session():
            ...
            self.assertEquals(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES), [])
    ...
    def testReuseConvWithWD(self):
        ...
        with self.test_session():
            ...
            self.assertEquals(len(variables.get_variables()), 2)
            self.assertEquals(len(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)), 1)
            ops.conv2d(images, 32, [3, 3], weight_decay=0.01, scope="conv1", reuse=True)
            self.assertEquals(len(variables.get_variables()), 2)
            self.assertEquals(len(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)), 1)
...
class FCTest(tf.test.TestCase):
    ...
    def testCreateFCWithWD(self):
        ...
        with self.test_session() as sess:
            ...
            wd = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)[0]
            self.assertEquals(wd.op.name, "FC/weights/Regularizer/L2Regularizer/value")
            sess.run(tf.global_variables_initializer())
            self.assertTrue(sess.run(wd) <= 0.01)
...
class OneHotEncodingTest(tf.test.TestCase):
    ...
    def testOneHotEncoding(self):
        with self.test_session():
            labels = tf.constant([0, 1, 2])
            one_hot_labels = tf.constant([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
            output = ops.one_hot_encoding(labels, num_classes=3)
...
class FlattenTest(tf.test.TestCase):
    def testFlatten4D(self):
        ...
        with self.test_session():
            ...
            self.assertEquals(output.get_shape().num_elements(), images.get_shape().num_elements())
    ...
    def testFlattenBatchSize(self):
        ...
        with self.test_session() as sess:
            ...
            self.assertEquals(output.get_shape().as_list(), [None, height * width * 3])
...
class BatchNormTest(tf.test.TestCase):
    ...
    def testUpdateOps(self):
        ...
        with self.test_session():
            ...
            self.assertEquals(update_moving_mean.op.name, "BatchNorm/AssignMovingAvg")
    ...
    def testCreateMovingVars(self):
        ...
        with self.test_session():
            ...
            moving_mean = tf.get_collection("moving_vars", "BatchNorm/moving_mean")
            self.assertEquals(len(moving_mean), 1)
            self.assertEquals(moving_mean[0].op.name, "BatchNorm/moving_mean")
            moving_variance = tf.get_collection("moving_vars", "BatchNorm/moving_variance")
    ...
    def testEvalMovingVars(self):
        ...
        with self.test_session() as sess:
            ...
            init_assigns = [tf.assign(moving_mean, expected_mean), tf.assign(moving_variance, expected_var)]
