...
def get_inception_score(sess, images, pred_op):
    ...
    res_mean = np.mean(scores).item()
    res_std = np.std(scores).item()

    print("mean:", "%.2f" % res_mean, "std:", "%.2f" % res_std)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % FLAGS.gpu):
                ...
                known_logits = tf.div(known_logits, tf.constant(0.5980541706085205))
                ...
                saved_file = FLAGS.saved_file

                IS_mean, IS_std = get_inception_score(sess, images, pred_op)
