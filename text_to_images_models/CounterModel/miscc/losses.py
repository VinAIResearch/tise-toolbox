...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions, real_labels, fake_labels):
    ...
    fake_features = netD([fake_img.detach() for fake_img in fake_imgs])
    # loss
    #
...
def generator_loss(
    netD, image_encoder, fake_imgs, real_labels, words_embs, sent_emb, match_labels, cap_lens, class_ids
):
    ...
    region_features, cnn_code = image_encoder(fake_imgs[-1])  # Only calculate for highest resolution image
