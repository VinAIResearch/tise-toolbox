...
for positional_word in caption_data:

    if not os.path.exists(os.path.join(args.saved_dir, positional_word)):
        os.makedirs(os.path.join(args.saved_dir, positional_word))

    captions = list(
        zip(
            [item["caption_id"] for item in caption_data[positional_word]],
            [item["caption"] for item in caption_data[positional_word]],
        )
    )
    ...
    for cap_batch in tqdm(caption_batches):
        ...
        for cap_id, img in zip(cap_ids, images):
            img[-1].save(os.path.join(args.saved_dir, f"{positional_word}/{cap_id}.png"))
