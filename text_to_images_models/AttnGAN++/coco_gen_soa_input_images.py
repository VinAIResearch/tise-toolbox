...
def parse_args():
    ...
    parser.add_argument("--label_file_dir", default="", type=str)
...
list_label_files = os.listdir(args.label_file_dir)

for label_file in tqdm(list_label_files):
    folder_name = os.path.join(args.saved_dir, label_file.split(".")[0])
    if not os.path.exists(folder_name):
        os.mkdir(folder_name)

    # Open label caption file
    with open(os.path.join(args.label_file_dir, label_file), "rb") as f:
        captions = pickle.load(f)

    # Generate images
    caption_list = [i["caption"] for i in captions]

    batch_size = args.batch_size
    num_captions = len(caption_list)
    print(f"Label: {label_file}, num captions: {num_captions}")

    if num_captions % batch_size != 0:
        num_batches = int((num_captions / batch_size + 1))
    else:
        num_batches = int(num_captions / batch_size)

    caption_batches = []
    for i in range(num_batches):
        if i != num_batches - 1:
            cur_batch = caption_list[i * batch_size : (i + 1) * batch_size]
        else:
            cur_batch = caption_list[i * batch_size :]
        caption_batches.append(cur_batch)

    count = 0
    for cap_batch in caption_batches:
        images = generate_img_from_caption(cap_batch)
        images = [img[2] for img in images]
        for img in images:
            img.save("{}/image_{}.png".format(folder_name, count))
            count += 1
